...
class House
    def line(number)
        ...
        "This is the #{house_number.horse_and_hound}#{house_number.farmer_sowing}#{house_number.rooster_crowed}#{house_number.priest_all_shaven}#{man_all_tattered(number)}#{maiden_all_forelorn(number)}#{cow_with_crumpled(number)}#{dog_that_worried(number)}#{cat_that_killed(number)}#{rat_that_ate(number)}#{malt_that_lay(number)}house that Jack built.\n"
    end
